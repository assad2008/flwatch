...
def website_alarm_msm_content(itemname,itemobject,reason = '请求异常',level = 1):
	...
	middleTemp = " 监控类型:网页存活(http)，所在服务器:" + str(itemobject) + "。检查时间:" + str(monitorTime)
	template = topTemp + bottomTemp + middleTemp
	return template,subject
	
def ping_alarm_msm_content(itemname,itemobject,level = 1):
	itemname = itemname.encode('utf8')
	monitorTime = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
	if int(level) == 1:
	...
	middleTemp = "监控类型：ping 所在服务器:" + str(itemobject) + "。检查时间:" + str(monitorTime)
	template = topTemp + middleTemp + bottomTemp
	return template,subject
	
def custom_alarm_msm_content(itemname,itemobject,reason,level):
	itemname = itemname.encode('utf8')
	monitorTime = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
	if int(level) == 1:
		topTemp = "监控系统监控到您的监控项目:" + str(itemname) + " 连接失败。"
		...
		bottomTemp = "上一次故障原因:不稳定。"
		subject = "[恢复信息]" + str(topTemp)
	middleTemp = "监控类型:服务器指标，所在服务器:" + str(itemobject) + "，检查时间:" + str(monitorTime)
	template = topTemp + middleTemp + bottomTemp
	return template,subject
